...
static void cyttsp5_report_palm_event(struct cyttsp5_mt_data *md,unsigned long ids)
{
	struct device *dev = md->dev;
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	if(cd->easy_sleep_palm == 0) {
		dev_info(dev,"palm sleep is disable, return directly.\n");
		return;
	}
	if (md->mt_function.input_report)
...
/*** ZTEMT start ,add palm_sleep gesture,20150103***/
	...
	if(!fw_updating){
        ...
        /* For large area event */
            cyttsp5_report_palm_event(md,ids);
...
int cyttsp5_mt_probe(struct device *dev)
{
	...
	if (md->pdata->inp_dev_name)
		...
		md->input->name = CYTTSP568_MT_NAME;
