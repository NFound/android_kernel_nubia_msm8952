...
#define CYTTSP568_I2C_RST_GPIO    gpio_to_irq(rst_gpio_number)
#define CYTTSP568_I2C_IRQ_GPIO    gpio_to_irq(irq_gpio_number)

#define CYTTSP568_FIRMWARE_VERSION     0x6C10
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP568_PLATFORM_TTCONFIG_UPGRADE
		...
		int on, struct device *dev)
{
	...
	if(!gpio_is_valid(rst_gpio_number))
		return -ENODEV;
   	 rc = gpio_request(rst_gpio_number, "CYTTSP568_I2C_RST_GPIO");
    	if (rc < 0) 
    	{
	...
	rc = gpio_request(irq_gpio_number, "CYTTSP568_I2C_IRQ_GPIO");
	if (rc < 0) {
			printk("Failed request CYTTSP568_I2C_IRQ_GPIO.\n");
			return rc;
		}
...
/*** ZTEMT Added by lixin, 2015/08/07***/
char ztemt_target[100]="NX500J";
static int  ztemt_target_type(char *param)
{
    memcpy(ztemt_target, param, strlen(param));
    return 0;
}
early_param("androidboot.serialno", ztemt_target_type);
...
int cyttsp_check_version(struct cyttsp5_core_platform_data *pdata, struct device *dev)
{
    ...
    dev_info(dev, " ztemt_target is %s\n", ztemt_target);
    switch (pdata->fw_ver_ic & 0xFF00) {
        case 0x0C00:
            dev_info(dev, "%s: For FW upgarde from 0C to 6C\n", __func__);
            pdata->fw_ver_dr = CYTTSP568_FIRMWARE_VERSION;
            return 1;
            break;
        case 0x6C00:
            pdata->fw_ver_dr = CYTTSP568_FIRMWARE_VERSION;
