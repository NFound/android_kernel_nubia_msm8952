...
static int nubia_set_cabc(int cabc_val)
{
	...
	nubia_set_ce_cabc(nubia_disp_val.saturation,cabc_val);
	return ret;
}
...
static int nubia_set_saturation(int sat_val)
{
	...
	switch (sat_val) {
		...
		case SATURATION_OFF:
                        if (nubia_mdss_dsi_ctrl->ce_cmds_off.cmd_cnt)
                                ret = nubia_mdss_dsi_panel_cmds_send(nubia_mdss_dsi_ctrl, &nubia_mdss_dsi_ctrl->ce_cmds_off);
			break;
 		case SATURATION_SOFT:
	...
	nubia_set_ce_cabc(sat_val,nubia_disp_val.cabc);
	return ret;
}
	...
	struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	NUBIA_DISP_DEBUG("colortmp debug val = %d\n", val);

	if (val == 1)
		ret = nubia_mdss_pcc_config(&nubia_colortmp_debug);
