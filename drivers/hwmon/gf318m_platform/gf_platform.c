...
static int  ztemt_hw_config_get(char *param)
{
    memcpy(ztemt_config_version, param, strlen(param));
    return 0;
}
early_param("config_setup", ztemt_hw_config_get);
#endif
...
#ifndef CONFIG_ZTEMT_N940_FP_COMPATIBLE
#ifdef CONFIG_ZTEMT_FP_COMPATIBLE
	GF_LOG_INFO("dev config is %s\n", ztemt_config_version_A);
    if ((strcmp(pcb_version, "MB_A")) && strcmp(ztemt_fp_goodix, ztemt_fp_gpio_target))
        return -1;
#endif
#endif
