...
static int aw2013_vreg_enable(struct aw2013_reg_data *vreg)
{
	...
	if (!vreg->is_enabled) {
		/* Set voltage level */
		ret = aw2013_vreg_set_voltage(vreg, vreg->low_vol_level,
						vreg->high_vol_level);
		if (ret)
			return ret;
	}
...

{
	...
	aw2013_data->vreg_data->irq_gpio = of_get_named_gpio(client->dev.of_node,
		"aw2013-irq-gpio", 0);
	pr_err("nubia_debug:%s aw2013-irq-gpio = %d\n",__func__, aw2013_data->vreg_data->irq_gpio);
	if(aw2013_data->vreg_data->irq_gpio >= 0)	{
		ret = gpio_request(aw2013_data->vreg_data->irq_gpio,"aw2013-irq-gpio");
		if (ret < 0) {
			pr_err("%s: request gpio %d failed.\n", __func__, aw2013_data->vreg_data->irq_gpio);
		}
		ret = gpio_direction_input(aw2013_data->vreg_data->irq_gpio);
    }
